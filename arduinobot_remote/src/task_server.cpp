...
private:
  ...
  void execute(const std::shared_ptr<rclcpp_action::ServerGoalHandle<arduinobot_msgs::action::ArduinobotTask>> goal_handle)
  {
    ...
    if (goal_handle->get_goal()->task_number == 0) // Wake
    {
      arm_joint_goal = {0.0, 0.0, 0.0};
      gripper_joint_goal = {-0.7, 0.7};
    }
    else if (goal_handle->get_goal()->task_number == 1) // Pick
    {
      arm_joint_goal = {-1.14, -0.6, -0.07};
      gripper_joint_goal = {0.0, 0.0};
    }
    else if (goal_handle->get_goal()->task_number == 2) // Sleep
    {
      arm_joint_goal = {0.0,0.0,-0.9};
      gripper_joint_goal = {0.0, 0.0};
    }
    else if (goal_handle->get_goal()->task_number == 3) // Move
    {
      arm_joint_goal = {1.5,-0.6,-0.07};
      gripper_joint_goal = {0.0, 0.0};
    }
    else if (goal_handle->get_goal()->task_number == 4) // Place
    {
      arm_joint_goal = {1.5,-0.6,-0.07};
      gripper_joint_goal = {-0.7, 0.7};
    }
